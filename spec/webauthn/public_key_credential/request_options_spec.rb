...
RSpec.describe WebAuthn::PublicKeyCredential::RequestOptions do
  ...
  context 'client timeout' do
    it 'has a default client timeout' do
      expect(request_options.timeout).to(eq(120_000))
    end
    ...
    context 'when client timeout is configured' do
      ...
      it 'updates the client timeout' do
        expect(request_options.timeout).to(eq(60_000))
      end
    end
  end
  ...
  it 'has everything' do
    options = WebAuthn::PublicKeyCredential::RequestOptions.new(
      rp_id: 'rp-id',
      timeout: 10_000,
      allow_credentials: [{ type: 'public-key', id: 'credential-id', transports: %w[usb nfc] }],
    ...
    expect(hash[:allowCredentials]).to eq([{ type: 'public-key', id: 'credential-id', transports: %w[usb nfc] }])
