...
RSpec.describe WebAuthn::PublicKeyCredential::CreationOptions do
  ...
  context 'client timeout' do
    it 'has a default client timeout' do
      expect(creation_options.timeout).to(eq(120_000))
    end

    context 'when client timeout is configured' do
      before do
        WebAuthn.configuration.credential_options_timeout = 60_000
      end

      it 'updates the client timeout' do
        expect(creation_options.timeout).to(eq(60_000))
      end
    end
  end
  ...
  it 'has everything' do
    options = WebAuthn::PublicKeyCredential::CreationOptions.new(
      ...
      timeout: 10_000,
      exclude_credentials: [{ type: 'public-key', id: 'credential-id', transports: %w[usb nfc] }],
    ...
    expect(hash[:excludeCredentials]).to eq([{ type: 'public-key', id: 'credential-id', transports: %w[usb nfc] }])
